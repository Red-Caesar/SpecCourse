...
def load_calibration_dataset(tokenizer: AutoTokenizer, num_calibration_samples: int=512):
...
def quantize_model(
    ...
    for key in iteration_parameters["keys"]:
        for values in iteration_parameters["values_per_keys"]:
            for value in values:
                model_output_dir = Path(output_dir) / f"{model_name.split('/')[-1]}-{key}-{value}"
                ...
                try:
                    ...
                    recipe = function_map[quant_method](**{**static_parameters, **{key: value}})
                    ...
                    logger.error(f"Error processing model {model_name} with {key} {value}: {e}")
                    if 'model' in locals():
                        del model
...
def main():
    ...
    parser.add_argument("--output-dir", type=str, default="./models", help="Output directory for quantized models")
    ...
    logger = setup_logger(output_dir, "quantization")
    ...
    for model_name in models:
        for quant_method, quant_args in quant_methods.items(): 
            try:
                ...
                logger.error(f"Error processing model {model_name} with method {quant_method}: {e}")
                continue
